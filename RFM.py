...
def RFM():
    ...
    InlineData['TotalSum'] = InlineData['Quantity'] * InlineData['UnitPrice']# Create snapshot date
    snapshot_date = InlineData['InvoiceDate'].max() + timedelta(days=1)
    print(snapshot_date)# Grouping by CustomerID
    data_process = InlineData.groupby(['CustomerID']).agg({
